...
define(['N/record', 'N/search', 'N/runtime', 'N/error',
    './AMY MR Fulfillment Response Common', 'N/format'
], function(record, search, runtime, error, common, format) {
    var _this = {};
    ...
    function getInputData() {
        ...
        var scriptParams = getScriptConfiguration();
        log.debug('getInputData - slFilters: ' + scriptParams.filterObject);
		var slFilters = JSON.parse(scriptParams.filterObject);
        var recId = slFilters.recId;
        ...
        var filter = [
            ["isinactive", "is", "F"],
            "AND",
            ["custrecord_afr_order.mainline", "is", "T"],
            "AND",
            ["custrecord_afr_status", "anyof", "1", "3"],
            "AND",
            [
                [
                    ["custrecord_afr_order.type", "anyof", "TrnfrOrd"], "AND", ["custrecord_afr_order.status", "anyof", "TrnfrOrd:B"]
                ], "OR", ["custrecord_afr_order.type", "anyof", "SalesOrd"]
            ]
        ];
        if (recId != undefined && recId != null && recId != '') {
            recId = recId.replace(/\s/g, '');	// remove all white spaces
			recId = recId.split(",");		// convert to an array
			var recFilter = ["internalid","anyof"];
			recFilter = recFilter.concat(recId);
			filter.push("AND");
			filter.push(recFilter);
			log.debug('recFilter', recFilter);
        }
        ...
        if (brand != undefined && brand != null && brand != '') {
			...
			var brandFilter = ["custrecord_afr_order.class", "anyof"];
			brand = brand.split(",");
        ...
        if (location != undefined && location != null && location != '') {
			...
			var locFilter = ["custrecord_afr_order.location", "anyof"];
			location = location.split(",");
			for (var i=0; i<location.length;i++){
				locFilter.push(location[i]);
			};
            filter.push("AND");
            filter.push(locFilter);
        }
        var amyFulRes = search.create({
            type: "customrecord_amy_fulfillment_response",
            filters: filter,
            columns: [
                search.createColumn({
                    name: "custrecord_afr_order",
                    label: "Order"
                }),
                search.createColumn({
                    name: "custrecord_afr_fulfillment",
                    label: "Fulfillment"
                }),
                search.createColumn({
                    name: "custrecord_afr_billing_transaction",
                    label: "Billing Transaction"
                }),
                search.createColumn({
                    name: "custrecord_afr_status",
                    label: "Status"
                }),
                search.createColumn({
                    name: "statusref",
                    join: "CUSTRECORD_AFR_ORDER",
                    label: "Status"
                }),
                search.createColumn({
                    name: "paymentmethod",
                ...
                search.createColumn({
                    name: "custbody_cb_order_type",
                    join: "CUSTRECORD_AFR_ORDER",
                    label: "Order Type"
                }),
                search.createColumn({
                    name: "custrecord_afr_message",
                    label: "Message"
                }),
                search.createColumn({
                    name: "custrecord_afr_failure_count",
                    label: "Failure Count"
                }),
                search.createColumn({
                    name: "custentity_acb_edi_partner",
                    join: "CUSTRECORD_AFR_CUSTOMER",
                    label: "EDI Partner"
                }),
                search.createColumn({
                    name: "otherrefnum",
                ...
                search.createColumn({
                    name: "custrecord_afr_error_message",
                    label: "Error Message"
                })
            ]
        });
        var searchResultCount = amyFulRes.runPaged().count;
        log.debug("amyFulRes result count", searchResultCount);
        log.debug('amyFulRes Search Object:', amyFulRes);
        return amyFulRes;
        /*return {
        	type: 'search',
    ...
    function fulfillOrder(fulfillmentResponse) {
        ...
        fulfillment.setValue({
            fieldId: 'trandate',
            value: format.parse({
                value: new Date(fulfillmentResponse.transactionDate),
        ...
        fulfillment.setValue({
            ...
            value: format.parse({
                value: new Date(fulfillmentResponse.shipDate),
                type: format.Type.DATE
            })
        });
        fulfillment.setValue({
        ...
        fulfillment.setValue({
            fieldId: 'custbody7',
            value: fulfillmentResponse.message.carrier
        });
        ...
        if (common.isQuietLogisticsLocation(fulfillmentResponse.location) || common.isPFSLocation(fulfillmentResponse.location)) {
            if (fulfillmentResponse.message.carrier_tracking_number) {
                ...
                cartons.forEach((val, line) => {
                    ...
                    fulfillment.setSublistValue({
                        ...
                        value: description.substring(0, Math.min(999, description.length))
                    });
                });
            } else {
                log.error('no carton header information (carrier, tracking number, etc.) found for this order. No package was created.', {
                    fulfillmentResponse
                });
            }
        } else {
            ...
            fulfillment.setSublistValue({
                ...
                line: 0,
                fieldId: 'packagetrackingnumber',
                value: fulfillmentResponse.message.carrier_tracking_number
            });
        }
        ...
        for (var i = 0; i < fulfillment.getLineCount({
                sublistId: 'item'
            }); ++i) {
            var sku = fulfillment.getSublistValue({
                sublistId: 'item',
                line: i,
                fieldId: 'itemname'
            });
            var qtyToShip = parseInt(fulfillment.getSublistValue({
                sublistId: 'item',
                line: i,
            ...
            if (qtyToShip > 0) {
                if (shippedQuantityRemaining[sku] > 0) {
                    ...
                    fulfillment.setSublistValue({
                        ...
                        line: i,
                        fieldId: 'location',
                        value: fulfillmentResponse.location
                    });
        ...
        for (var sku in shippedQuantityRemaining) {
            if (shippedQuantityRemaining[sku] > 0) {
                ...
                if (itemFirstFulfillmentLine[sku] != undefined) {
                    ...
                    var alreadyShippedQuantity = parseInt(fulfillment.getSublistValue({
                        sublistId: 'item',
                        line: itemFirstFulfillmentLine[sku],
                        fieldId: 'quantity'
                    })) || 0;
        ...
        if (fulfillmentResponse.orderType !== 'transferorder') {
            try {
                record.submitFields({
                    ...
                    values: {
                        'custbody_cb_3pl_ship_date_so': format.parse({
                            value: new Date(fulfillmentResponse.shipDate),
                            type: format.Type.DATE
                        })
                    }
    ...
    function billOrder(fulfillmentResponse) {
        ...
        } else if (fulfillmentResponse.orderType == 'B2C' || fulfillmentResponse.orderType == 'INTL') {
        ...
        if (billOrder && fulfillmentResponse.orderType !== 'transferorder') {
            ...
            if (limitBillingToOrder) {
                for (var i = 0; i < billingTransaction.getLineCount({
                        sublistId: 'item'
                    }); ++i) {
                    if (billingTransaction.getSublistValue({
                            sublistId: 'item',
                            line: i,
                            fieldId: 'itemtype'
                        }) != 'EndGroup') {
                        var orderLineDetails = limitBillingToOrder[billingTransaction.getSublistValue({
                            sublistId: 'item',
                            line: i,
                            fieldId: 'orderline'
                        })];
            ...
            var discountAmount = parseFloat(billingTransaction.getValue({
                fieldId: 'discounttotal'
            })) || 0;
            var discountItem = billingTransaction.getValue({
            ...
            if (discountItem) {
                if (canProrateDiscount(fulfillmentResponse, discountItem, discountAmount)) {
                    ...
                    for (var i = 0; i < billingTransaction.getLineCount({
                        ...
                        var itemType = billingTransaction.getSublistValue({
                            sublistId: 'item',
                            line: i,
                            fieldId: 'itemtype'
                        });
                        ...
                        if (itemType == 'Assembly' || itemType == 'InvtPart') {
                            var lineExtendedPrice = parseFloat(billingTransaction.getSublistValue({
                                sublistId: 'item',
                                line: i,
                                fieldId: 'amount'
                            })) || 0;
                            ...
                            itemsToDiscount[i] = {
                                ...
                                quantity: billingTransaction.getSublistValue({
                                    sublistId: 'item',
                                    line: i,
                                    fieldId: 'quantity'
                                })
    ...
    function getScriptConfiguration() {
        ...
        return {
            senderId: scriptObj.getParameter({
                name: 'custscript_amy_frp_email_sender'
            }),
            templateId: parseInt(scriptObj.getParameter({
                name: 'custscript_amy_frp_invoice_template'
            })),
            ccAddress: scriptObj.getParameter({
                name: 'custscript_amy_frp_carbon_copy'
            }),
            filterObject: scriptObj.getParameter({
                name: 'custscript_amy_filter_object'
            })
        };
    }
    return {
